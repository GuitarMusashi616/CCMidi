...
local function readVLQ(stream)
  ...
  repeat
    ...
    value = bit32.band(bit32.bor(value, byte), 0x7F)
    length = length + 1
  until byte < 0x80
  return value, length
end
...
local midiEvent = {
  [0x80] = function(stream, callback, channel, fb)
    local key, velocity = string_unpack("I1I1", fb .. stream:read(1))
  ...
  [0xA0] = function(stream, callback, channel, fb)
    local key, pressure = string_unpack("I1I1", fb .. stream:read(1))
    callback("keyPressure", channel, key, pressure / 0x7F)
    return 2
  end,
  [0xB0] = function(stream, callback, channel, fb)
    local number, value = string_unpack("I1I1", fb .. stream:read(1))
    if number < 120 then
  ...
  [0xE0] = function(stream, callback, channel, fb)
    local lsb, msb = string_unpack("I1I1", fb .. stream:read(1))
...
local function readChunkInfo(stream)
  ...
  return string_unpack(">c4I4", chunkInfo)
end
...
local function readHeader(stream, callback, chunkLength)
  ...
  local format, tracks, division = string_unpack(">I2I2I2", header)
...
local function processEvent(stream, callback, runningStatus)
  ...
  if status >= 0x80 and status < 0xF0 then
    length = length + midiEvent[bit32.band(status, 0xF0)](stream, callback, bit32.band(status, 0xF0) + 1, firstByte) 
